...
pathwayProcessor.prototype.bp3getPathwayDesc = function(H_xml) {
	var p = H_xml.getElementsByTagNameNS(this.bp, 'Pathway');
	var A_cmt = [];
	for (var i=0 ; i<p.length ; i++) {
		var c = p[i].getElementsByTagNameNS(this.bp, 'comment');
		for (var j=0 ; j<c.length ; j++)
			A_cmt.push(c[j].textContent);
	}
	return A_cmt;
}
...
pathwayProcessor.prototype.bp3annotate = function() {
	...
	var F_anno = function(I) {
		...
		I.ref = anno_raw;
		//
		I.ref.forEach(function(v) {
			if (v.db == 'enzyme nomenclature')
				I.type = 'compound';
		});
	};
	// Check children
...
pathwayProcessor.prototype.bp3getEdges = function(xml) {
	...
	var A_target = this.bp3getElementsByTagNames('BiochemicalReaction', 'ComplexAssembly', 'Catalysis', 'Control', 'Conversion', 'Degradation', 'Modulation', 'TemplateReactionRegulation', 'Transport', 'TransportWithBiochemicalReaction');
...
pathwayProcessor.prototype.A_annoURL = {
	...
	'enzyme nomenclature':		['anno/pair.php', '|', 'EC', 'https://enzyme.expasy.org/EC/'],
	'go':						['anno/pair.php', '|'],
	'gene ontology':			['anno/pair.php', '|'],
	'psi-mod':					['anno/pair.php', '|'],
	'lipid maps':				['anno/pair.php', '|'],
	'mirbase sequence':			['anno/pair.php', '|'],
	'panther pathway component':['anno/pair.php', '|', 'PANTHER', 'http://www.pantherdb.org/pathway/pathCatDetail.do?clsAccession='],
	'protein data bank':		['anno/pair.php', '|', 'PDB', 'https://www.rcsb.org/pdb/explore/explore.do?structureId='],
	'kegg':						['keggid2genesymbol.php', '|', 'KEGG', 'http://www.genome.jp/dbget-bin/www_bget?hsa:'],
